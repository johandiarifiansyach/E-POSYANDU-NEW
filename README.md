...
```text
frontend/              Aplikasi native HTML5, CSS, dan TypeScript
public/                Aset statis, service worker, header, dan redirect
backend-rust-worker/   API Rust untuk Cloudflare Workers
...
Untuk menjalankan Worker di komputer sendiri, salin `backend-rust-worker/.dev.vars.example` menjadi `backend-rust-worker/.dev.vars`, isi nilai lokal, lalu jalankan `npm run worker:dev`. Berkas `.dev.vars` tidak pernah masuk Git.

Dokumentasi API tersedia dari endpoint `/api/v1/openapi.json`; konfigurasi backend ada di [backend-rust-worker/README.md](backend-rust-worker/README.md).
