...
type ChangeHistoryPageProps = {
    ...
    pageState?: PageState<{ items: ChangeHistoryItem[]; total: number }>;
    loading?: boolean;
    error?: string | null;
    currentPage?: number;
    total?: number;
...
export default function ChangeHistoryPage({
    ...
    const resolvedState: PageState<{ items: ChangeHistoryItem[]; total: number }> = pageState ?? (loading
        ? { status: 'loading' }
        : error
            ? { status: 'error', message: error }
            : { status: 'success', data: { items: changeLogs, total } });
    const resolvedLogs = resolvedState.status === 'success' ? resolvedState.data.items : changeLogs;
    const resolvedTotal = resolvedState.status === 'success' ? resolvedState.data.total : total;
    const resolvedLoading = resolvedState.status === 'loading';
    const resolvedError = resolvedState.status === 'error' ? resolvedState.message : null;
    ...
    const sortedChangeLogs = [...resolvedLogs].sort((left, right) => {
    ...
    const startRow = resolvedTotal === 0 ? 0 : ((currentPage - 1) * pageSize) + 1;
    const endRow = Math.min(resolvedTotal, startRow + sortedChangeLogs.length - 1);
    return (Native.createElement("div", { className: "apple-page space-y-6" },
        ...
        }))), !resolvedLoading && !resolvedError && resolvedTotal > 0 && Native.createElement("div", { className: "ios-table-footer app-card flex flex-col items-center justify-between gap-4 p-4 sm:flex-row" },
            Native.createElement("span", { className: "text-xs font-medium text-slate-500" }, "Menampilkan ", startRow, " - ", endRow, " dari ", resolvedTotal, " riwayat"),
            Native.createElement(Pagination, { currentPage: currentPage, totalPages: totalPages, disablePrevious: currentPage <= 1, disableNext: currentPage >= totalPages, onPrevious: () => onPageChange?.(Math.max(1, currentPage - 1)), onNext: () => onPageChange?.(Math.min(totalPages, currentPage + 1)) }))));
}
