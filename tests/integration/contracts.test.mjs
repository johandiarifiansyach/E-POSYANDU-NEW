...
test('deployment Oracle mengisolasi layanan dan tidak menaruh secret dalam image', async () => {
  ...
  assert.match(compose, /\/run\/e-posyandu\/origin-cert\.pem:\/run\/secrets\/eposyandu-origin-cert\.pem:ro,Z/);
  assert.match(compose, /\/run\/e-posyandu\/origin-key\.pem:\/run\/secrets\/eposyandu-origin-key\.pem:ro,Z/);
  assert.match(compose, /expose:\s+- "8443"/);
  ...
  assert.match(caddy, /https:\/\/api\.eposyandu\.app:8443, https:\/\/nutrition\.eposyandu\.app:8443/);
  assert.match(caddy, /tls \/run\/secrets\/eposyandu-origin-cert\.pem \/run\/secrets\/eposyandu-origin-key\.pem/);
  ...
  assert.match(vaultMaterializer, /OCI_SECRET_ORIGIN_CERT_ID/);
  assert.match(vaultMaterializer, /OCI_SECRET_ORIGIN_KEY_ID/);
  assert.match(vaultMaterializer, /fetch_base64_pem_secret/);
  assert.match(vaultMaterializer, /origin-cert\.pem/);
  assert.match(vaultMaterializer, /origin-key\.pem/);
