...
test('migration database berurutan dan tercatat sampai versi terbaru', async () => {
  ...
  assert.equal(files.at(-1), '053_child_nik_source_filter.sql');
...
test('deployment Oracle mengisolasi layanan dan tidak menaruh secret dalam image', async () => {
  ...
  assert.doesNotMatch(compose, /origin-cert\.pem:\/run\/secrets/);
  assert.doesNotMatch(compose, /origin-key\.pem:\/run\/secrets/);
  assert.match(compose, /expose:\s+- "8088"/);
  ...
  assert.doesNotMatch(caddy, /tls \/run\/secrets\/eposyandu-origin-cert\.pem/);
