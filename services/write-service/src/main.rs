use e_posyandu_oracle_domain::{WriteDomain, request_from_proto, response_to_proto};
use e_posyandu_proto::proto::platform::v1::{
    ServiceRequest, ServiceResponse,
    write_service_server::{WriteService, WriteServiceServer},
};
use e_posyandu_proto::transport::{ListenAddress, bind_unix, parse_listen_address};
use std::{env, io, sync::Arc, time::Duration};
use tokio_stream::wrappers::UnixListenerStream;
use tonic::{
    Request, Response, Status,
    metadata::{Ascii, MetadataValue},
    service::Interceptor,
    transport::Server,
};

const TOKEN_HEADER: &str = "x-eposyandu-service-token";

#[derive(Clone)]
struct AuthInterceptor {
    token: MetadataValue<Ascii>,
}

impl Interceptor for AuthInterceptor {
    fn call(&mut self, request: Request<()>) -> Result<Request<()>, Status> {
        let supplied = request
            .metadata()
            .get(TOKEN_HEADER)
            .ok_or_else(|| Status::unauthenticated("Token service write tidak ditemukan."))?;
        if supplied != &self.token {
            return Err(Status::unauthenticated("Token service write tidak valid."));
        }
        Ok(request)
    }
}

fn service_auth() -> Result<AuthInterceptor, io::Error> {
    let value = env::var("RUST_WORKER_SHARED_SECRET")
        .ok()
        .filter(|value| !value.trim().is_empty())
        .ok_or_else(|| io::Error::other("RUST_WORKER_SHARED_SECRET wajib diisi."))?;
    let token = value
        .parse()
        .map_err(|_| io::Error::other("RUST_WORKER_SHARED_SECRET harus ASCII."))?;
    Ok(AuthInterceptor { token })
}

#[derive(Clone)]
struct WriteGrpc {
    domain: Arc<WriteDomain>,
}

#[tonic::async_trait]
impl WriteService for WriteGrpc {
    async fn handle(
        &self,
        request: Request<ServiceRequest>,
    ) -> Result<Response<ServiceResponse>, Status> {
        let input = request.into_inner();
        let http = request_from_proto(&input).map_err(|status| {
            Status::invalid_argument(format!("Request write tidak valid: {status}"))
        })?;
        Ok(Response::new(
            response_to_proto(self.domain.handle(http).await).await,
        ))
    }
}

fn address() -> Result<ListenAddress, io::Error> {
    parse_listen_address(
        &env::var("WRITE_GRPC_ADDR").unwrap_or_default(),
        "unix:///run/e-posyandu/write.sock",
        "WRITE_GRPC_ADDR",
    )
}

async fn shutdown_signal() {
    let _ = tokio::signal::ctrl_c().await;
}

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    tracing_subscriber::fmt()
        .json()
        .with_env_filter(tracing_subscriber::EnvFilter::from_default_env())
        .init();
    let address = address()?;
    let domain = Arc::new(WriteDomain::from_env().await.map_err(io::Error::other)?);
    let cleanup_domain = domain.clone();
    tokio::spawn(async move {
        let mut interval = tokio::time::interval(Duration::from_secs(24 * 60 * 60));
        loop {
            interval.tick().await;
            if !cleanup_domain.cleanup_retention().await {
                eprintln!("pembersihan retensi data balita gagal");
            }
        }
    });
    let (reporter, health) = tonic_health::server::health_reporter();
    reporter
        .set_serving::<WriteServiceServer<WriteGrpc>>()
        .await;
    let server = Server::builder()
        .layer(tonic::service::InterceptorLayer::new(service_auth()?))
        .add_service(health)
        .add_service(WriteServiceServer::new(WriteGrpc { domain }));
    match address {
        ListenAddress::Tcp(address) => {
            server
                .serve_with_shutdown(address, shutdown_signal())
                .await?;
        }
        ListenAddress::Unix(path) => {
            let listener = bind_unix(&path)?;
            server
                .serve_with_incoming_shutdown(UnixListenerStream::new(listener), shutdown_signal())
                .await?;
        }
    }
    Ok(())
}
