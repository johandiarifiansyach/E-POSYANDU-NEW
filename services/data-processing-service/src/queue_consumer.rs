...
use crate::proto::{
    ...
    data_processing_worker_client::DataProcessingWorkerClient,
};
...
impl QueueConfig {
    pub fn new(input: QueueConfigInput) -> Result<Self, String> {
        ...
        Ok(Self {
            ...
            grpc_url: if input.grpc_url.trim().is_empty() {
                "unix:///tmp/e-posyandu/data-processing.sock".into()
            } else {
                input.grpc_url
            },
    ...
    fn from_env() -> Result<Option<Self>, String> {
        ...
        Self::new(QueueConfigInput {
            ...
            grpc_url: env::var("DATA_PROCESSING_GRPC_URL")
                .or_else(|_| env::var("GRPC_URL"))
                .unwrap_or_else(|_| "unix:///tmp/e-posyandu/data-processing.sock".into()),
            batch_size,
...
async fn process_message(
    ...
    let mut grpc = DataProcessingWorkerClient::connect(config.grpc_url.clone())
        .await
