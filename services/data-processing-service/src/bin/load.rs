...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let client = DataProcessingWorkerClient::connect(endpoint.clone()).await?;
        ...
        tasks.spawn(async move {
            ...
            let response = client
                .process_job(Request::new(ProcessJobRequest {
                    job_id: format!("load-{request_index}"),
                    kind: "system_sync".into(),
                    payload_json: json!({
                        "records": (0..items_per_request).map(|index| json!({
                            "resource": "children",
                            "operation": "update",
                            "documentId": format!("load-{request_index}-{index}"),
                            "idempotencyKey": format!("load-{request_index}-{index}-key"),
                            "version": 1,
                            "payloadJson": "{}"
                        })).collect::<Vec<_>>()
                    }).to_string(),
                ...
                .await?;
            if response.into_inner().status != "completed" {
                return Err(tonic::Status::internal("Status job tidak selesai."));
            }
