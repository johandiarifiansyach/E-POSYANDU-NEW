use e_posyandu_oracle_domain::{OperationsDomain, request_from_proto, response_to_proto};
...
impl Interceptor for AuthInterceptor {
    fn call(&mut self, request: Request<()>) -> Result<Request<()>, Status> {
        let supplied = request
            .metadata()
            .get(TOKEN_HEADER)
            .ok_or_else(|| Status::unauthenticated("Token service operations tidak ditemukan."))?;
        if supplied != &self.token {
            return Err(Status::unauthenticated(
                "Token service operations tidak valid.",
            ));
        }
        Ok(request)
    }
...
#[derive(Clone)]
struct OperationsGrpc {
    domain: std::sync::Arc<OperationsDomain>,
}

#[tonic::async_trait]
impl OperationsService for OperationsGrpc {
    async fn handle(
        &self,
        ...
        let http = request_from_proto(&input).map_err(|status| {
            Status::invalid_argument(format!("Request operations tidak valid: {status}"))
        })?;
        Ok(Response::new(
...
fn address() -> Result<ListenAddress, io::Error> {
    parse_listen_address(
        &env::var("OPERATIONS_GRPC_ADDR").unwrap_or_default(),
        "unix:///run/e-posyandu/operations.sock",
        "OPERATIONS_GRPC_ADDR",
    )
}
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let address = address()?;
    let domain = Arc::new(
        OperationsDomain::from_env()
            .await
            .map_err(io::Error::other)?,
    );
    let cleanup_domain = domain.clone();
    tokio::spawn(async move {
        let mut interval = tokio::time::interval(Duration::from_secs(24 * 60 * 60));
        loop {
            interval.tick().await;
            if !cleanup_domain.cleanup_retention().await {
                eprintln!("pembersihan retensi data balita gagal");
            }
        }
    });
    let (reporter, health) = tonic_health::server::health_reporter();
    reporter
        .set_serving::<OperationsServiceServer<OperationsGrpc>>()
        .await;
    let server = Server::builder()
        .layer(tonic::service::InterceptorLayer::new(service_auth()?))
        .add_service(health)
        .add_service(OperationsServiceServer::new(OperationsGrpc { domain }));
    match address {
