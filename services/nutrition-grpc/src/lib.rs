...
use proto::nutrition_worker_server::NutritionWorker;
use proto::{
    CalculateBatchRequest, CalculateBatchResponse, CalculateReportRequest, CalculateReportResponse,
    ExportRow, ImportRow, NormalizeSyncBatchRequest, NormalizeSyncBatchResponse,
    NutritionAssessment, NutritionItem, PrepareExportRequest, PrepareExportResponse,
    ProcessJobRequest, ProcessJobResponse, SyncRecord, ValidateImportRequest,
    ValidateImportResponse, ValidationIssue,
};

const MAX_BATCH_ITEMS: usize = 10_000;
const MAX_EXPORT_ROWS: usize = 100_000;
const MAX_EXPORT_COLUMNS: usize = 128;
const STANDARDS_JSON: &str = include_str!("../data/anthropometry.json");
...
fn standards() -> Result<&'static WhoStandards, Status> {
    static STANDARDS: OnceLock<Result<WhoStandards, String>> = OnceLock::new();
    STANDARDS
        .get_or_init(|| serde_json::from_str(STANDARDS_JSON).map_err(|error| error.to_string()))
        .as_ref()
        .map_err(|_| Status::internal("Standar antropometri tidak dapat dimuat."))
}
...
fn nutrition_status(score: Option<f64>, growth_type: &str) -> String {
...
fn validate_item(item: &NutritionItem, index: usize) -> Result<(), Status> {
    if !item.weight_kg.is_finite() || !(0.1..=60.0).contains(&item.weight_kg) {
        return Err(Status::invalid_argument(format!(
            "items[{index}].weight_kg harus antara 0,1 dan 60 kg."
        )));
    }
    if let Some(height) = item.height_cm
    ...
    if !matches!(item.sex.as_str(), "L" | "P") {
        return Err(Status::invalid_argument(format!(
            "items[{index}].sex harus L atau P."
        )));
    }
    Ok(())
}

fn assess_item(item: &NutritionItem, standards: &WhoStandards) -> NutritionAssessment {
    ...
    let tbu = item.height_cm.and_then(|height| {
        z_score(
            height,
            "TBU",
            item.age_months,
    ...
    let bbtb = item.height_cm.and_then(|height| {
        z_score(
            item.weight_kg,
            "BBTB",
            item.age_months,
    ...
    let imtu = item.height_cm.and_then(|height| {
        z_score(
            item.weight_kg,
            "IMTU",
            item.age_months,
...
pub fn calculate_batch(items: &[NutritionItem]) -> Result<CalculateBatchResponse, Status> {
    if items.len() > MAX_BATCH_ITEMS {
        return Err(Status::resource_exhausted(
            "Jumlah item melebihi batas 10.000 per batch.",
        ));
    }
    let standards = standards()?;
    let mut response = CalculateBatchResponse {
        total: items.len() as u64,
        ..CalculateBatchResponse::default()
    };
    for (index, item) in items.iter().enumerate() {
        validate_item(item, index)?;
        let assessment = assess_item(item, standards);
        if matches!(
            assessment.bbu_status.as_str(),
            "Berat Sangat Kurang" | "Berat Kurang"
        ) {
            response.underweight += 1;
        }
        if matches!(assessment.tbu_status.as_str(), "Sangat Pendek" | "Pendek") {
            response.stunting += 1;
        }
        if matches!(
            assessment.bbtb_status.as_str(),
            "Gizi Buruk" | "Gizi Kurang"
        ) {
            response.wasting += 1;
        }
        response.items.push(assessment);
    }
    Ok(response)
}
...
pub fn calculate_report(items: &[NutritionItem]) -> Result<CalculateReportResponse, Status> {
    let batch = calculate_batch(items)?;
    Ok(CalculateReportResponse {
        total: batch.total,
        valid: batch.total,
        underweight: batch.underweight,
        stunting: batch.stunting,
        wasting: batch.wasting,
        items: batch.items,
    })
}
...
impl From<NutritionJobItem> for NutritionItem {
    fn from(value: NutritionJobItem) -> Self {
        Self {
            weight_kg: value.weight_kg,
            height_cm: value.height_cm,
            age_months: value.age_months,
...
fn assessment_json(value: &NutritionAssessment) -> Value {
    json!({
        "rowNumber": value.row_number,
    ...
    match request.kind.as_str() {
        ...
        "nutrition_report" => {
            ...
            let items = payload
                .items
                .into_iter()
                .map(NutritionItem::from)
                .collect::<Vec<_>>();
            let result = calculate_report(&items)?;
            response.result_json = json!({
                "total": result.total,
                "valid": result.valid,
                ...
                "items": result.items.iter().map(assessment_json).collect::<Vec<_>>(),
            })
            .to_string();
        }
        "export_file" => {
...
impl NutritionWorker for NutritionService {
    ...
    async fn calculate_report(
        &self,
        request: Request<CalculateReportRequest>,
    ) -> Result<Response<CalculateReportResponse>, Status> {
        Ok(Response::new(calculate_report(
            &request.into_inner().items,
        )?))
    }
    ...
    async fn process_job(
        ...
        Ok(Response::new(process_job(request.into_inner())?))
    }
}
...
mod tests {
    ...
    #[test]
    fn calculates_normal_newborn_without_problem_counts() {
        let result = calculate_batch(&[newborn()]).expect("valid nutrition result");
    ...
    #[test]
    fn rejects_gram_weight_as_kilograms() {
        let mut item = newborn();
        item.weight_kg = 3200.0;
        let error = calculate_batch(&[item]).expect_err("invalid weight");
