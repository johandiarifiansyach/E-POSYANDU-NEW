use e_posyandu_oracle_domain::{ReadDomain, request_from_proto, response_to_proto};
...
impl Interceptor for AuthInterceptor {
    fn call(&mut self, request: Request<()>) -> Result<Request<()>, Status> {
        let supplied = request
            .metadata()
            .get(TOKEN_HEADER)
            .ok_or_else(|| Status::unauthenticated("Token service read tidak ditemukan."))?;
        if supplied != &self.token {
            return Err(Status::unauthenticated("Token service read tidak valid."));
        }
        Ok(request)
    }
}
...
impl ReadService for ReadGrpc {
    async fn handle(
        ...
        let http = request_from_proto(&input).map_err(|status| {
            Status::invalid_argument(format!("Request read tidak valid: {status}"))
        })?;
        Ok(Response::new(
...
fn address() -> Result<ListenAddress, io::Error> {
    parse_listen_address(
        &env::var("READ_GRPC_ADDR").unwrap_or_default(),
        "unix:///run/e-posyandu/read.sock",
        "READ_GRPC_ADDR",
    )
}
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let server = Server::builder()
        ...
        .add_service(health)
        .add_service(ReadServiceServer::new(ReadGrpc { domain }));
    match address {
