...
-- The dashboard joins measurements through the legacy-compatible child key.
-- Index that expression so the latest measurement lookup does not scan the
-- complete measurements table for every active child.
create index if not exists idx_measurements_linked_child_date_created
  on public.measurements (
    (coalesce(child_id, nullif(legacy_child_id, ''))),
...
-- Keep the dashboard contract stable while avoiding growth classification for
-- children without a current measurement. The old query called the LMS
-- lookup three times for every active child, which could hit statement_timeout
-- on a full-puskesmas dashboard.
...
values ('021', 'optimize dashboard stats indexes and conditional growth classification')
