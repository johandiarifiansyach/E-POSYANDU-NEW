// Shared age cohorts used by every child table and the dashboard.  Keep the
// machine values stable because they are part of the API/cache contract.
...
// Some programme pages intentionally expose a narrower cohort than the
// shared table/dashboard filter. Keep these lists derived from the canonical
// options so labels and machine values cannot drift apart.
export const EXCLUSIVE_BREASTFEEDING_AGE_GROUP_OPTIONS = AGE_GROUP_OPTIONS.filter(
  (option) => option.value === '0-5' || option.value === '6'
);

export const MPASI_AGE_GROUP_OPTIONS = AGE_GROUP_OPTIONS.filter(
  (option) => option.value === '6-23'
);
...
export function normalizeAgeGroupForTab(value: unknown, tab: unknown): AgeGroup {
  ...
  if (options.some((option) => option.value === value)) return value as AgeGroup;
  return options[0]?.value || DEFAULT_AGE_GROUP;
}
...
function ageInMonths(child: any, referenceDate: Date): number | null {
...
function isNewborn(child: any, referenceDate: Date): boolean {
  const birthValue = child?.tglLahir || child?.birth_date || child?.birthDate;
...
function isPremature(child: any): boolean {
...
export function matchesAgeGroup(child: any, group: unknown, referenceDate: Date): boolean {
