...
import { childMatchesNikStatus } from '../../config/nikFilters';
...
export const Dashboard = ({ user, onLogout }) => {
    ...
    // Age cohorts are page-scoped. General pages start at 0–59 months;
    // programme pages use their own fixed default cohort when entered.
    useEffect(() => {
        const pageDefault = activeTab === 'asi_eksklusif'
            ? '0-5'
            : activeTab === 'mpasi'
                ? '6-23'
                : DEFAULT_AGE_GROUP;
        setAgeGroup((current) => current === pageDefault ? current : pageDefault);
    }, [activeTab]);
    ...
    }, [activeTab, currentPage, dataRevision, filterMonth, filterYear, ageGroup, isServerPagedChildTab, itemsPerPage, nikStatus, searchTerm, sortOrder, viewDesa, viewPosyandu]);
    ...
    const rawDisplayData = isServerPagedChildTab
        ? []
        : getDisplayData()
            .filter(c => childMatchesNikStatus(c, nikStatus))
            .filter(c => c.nama.toLowerCase().includes(searchTerm.toLowerCase()) || c.nik.includes(searchTerm));
    ...
    return (Native.createElement("div", { ref: appShellRef, className: `app-shell font-sans text-slate-900 flex ${sidebarCollapsedRef.current ? 'is-sidebar-collapsed' : ''} ${isSidebarOpen ? 'is-mobile-sidebar-open' : ''}` },
