...
import { hasUsableAnalysis, pythonWeightGainStatus, requestMeasurementAnalysis, requestPythonAnthropometry } from '../api/analysisApi';
            ...
            loadingHistory ? (Native.createElement(DataTable, { className: "measurement-history-scroll", ariaLabel: "Memuat riwayat penimbangan" },
                ...
                Native.createElement("table", { className: "ios-data-table ios-measurement-table text-xs", "data-measurement-history-table": true },
                    ...
                    Native.createElement("tbody", null, monthlyHistory.map((item) => {
                        ...
                        // Do not hide a persisted Python result when a
                        // transient fallback left analysisPending=true. A
                        // skeleton is appropriate only while no derived field
                        // is available for this history row.
                        const analysisLoading = Boolean(
                            (item.analysisPending || item.analysis_pending || pythonHistoryLoading)
                            && !hasUsableAnalysis(pythonStatus)
                        );
