/** React-owned child form rules. */
// @ts-nocheck
import { DATA_WILAYAH, ROLES } from '../../config/dashboard';
import { formatChildName, normalizeDecimalInput, parseLocaleNumberForRange } from '../../shared/formatters';
...
export function generateTemporaryKk() { return `350904${randomDigits(10)}`; }

const RANDOM_TEMPORARY_POSYANDU_CODES = new Set(['61', '98', '99']);
function getPosyanduNumber(posyandu) { const text = String(posyandu || '').trim(); return text.match(/(\d+)\s*$/)?.[1] || text.match(/\d+/)?.[0] || ''; }
function getTemporaryPosyanduCode(posyandu) { const number = getPosyanduNumber(posyandu); return number ? String(Number(number) % 100).padStart(2, '0') : '00'; }
function randomTemporaryPosyanduCode() { return String(10 + Math.floor(Math.random() * 51)).padStart(2, '0'); }
function getExistingNiks(allChildren, currentId) {
  return new Set((Array.isArray(allChildren) ? allChildren : [])
    ...
    .map((nik) => String(nik || '').trim()).filter((nik) => /^\d{16}$/.test(nik)));
}

export function generateTemporaryNik(data, allChildren = []) {
  const [year = '', month = '', day = ''] = String(data?.tglLahir || '').split('-');
  const birthDate = /^\d{4}$/.test(year) && /^\d{2}$/.test(month) && /^\d{2}$/.test(day) ? `${day}${month}${year.slice(-2)}` : randomDigits(6);
  ...
  if (!RANDOM_TEMPORARY_POSYANDU_CODES.has(posyanduCode) && !existingNiks.has(deterministicNik)) return deterministicNik;
...
export function createInitialChildForm(user) {
  const defaultDesa = user.role === ROLES.KADER || user.role === ROLES.BIDAN ? user.desa || Object.keys(DATA_WILAYAH)[0] : Object.keys(DATA_WILAYAH)[0];
  const defaultPosyandu = user.role === ROLES.KADER ? user.posyandu || DATA_WILAYAH[defaultDesa][0] : DATA_WILAYAH[defaultDesa][0];
  return { nama: '', nik: '', anakKe: '', tglLahir: '', jk: '', noKK: '', hasKK: true, hasNIK: true, usiaKehamilan: '', bbLahir: '', pbLahir: '', lkLahir: '', bukuKIA: '', bukuKIAKecil: '', imd: '', namaOrtu: '', nikOrtu: '', noHpOrtu: '', alamat: '', rt: '', rw: '', desa: defaultDesa, posyandu: defaultPosyandu };
}
...
export function validateChildBirthMeasurements({ bbLahir, pbLahir, lkLahir }) {
  ...
  const messages = { bbLahir: 'Berat lahir harus diisi dalam kilogram, misalnya 3,2 kg. Jangan masukkan 3200 gram.', pbLahir: 'Panjang lahir harus diisi desimal yang valid, misalnya 49,5 cm.', lkLahir: 'Lingkar kepala lahir harus diisi desimal yang valid, misalnya 33,2 cm.' };
  for (const key of Object.keys(values)) if (values[key] === null) return { ok: false, message: messages[key] };
