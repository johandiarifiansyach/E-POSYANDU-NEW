/**
 * Client-error reporter used by the React bootstrap.
 *
 * This intentionally lives in the React application rather than importing
 * the legacy DOM runtime.  The reporter is a small infrastructure service:
 * it creates one accessible panel, keeps the latest problem payload, and
 * delegates the actual submission to the authenticated API client.
 */
type ProblemPayload = {
  error: unknown;
  source: string;
...
function buildPayload(error: unknown, source: string): ProblemPayload {
  ...
  return { error, source, route, message, code: hashCode(`${source}|${route}|${message}`), occurredAt: new Date() };
}

function detailText(problem: ProblemPayload) {
  return [
...
function ensurePanel(options: ProblemReporterOptions) {
  ...
  const report = makeButton('Laporkan Masalah', 'problem-reporter-btn is-primary', async () => {
    if (!payload) return;
  ...
  reportButton = report;
  const copy = makeButton('Salin Detail', 'problem-reporter-btn', async () => {
    if (!payload) return;
    try {
      await navigator.clipboard.writeText(detailText(payload));
  ...
  const close = makeButton('Tutup', 'problem-reporter-btn', removePanel);
...
export function setupProblemReporter(options: ProblemReporterOptions) {
  return {
    capture(error: unknown, source: string) {
      ...
      ensurePanel(options);
      if (descriptionText) descriptionText.textContent = payload.message;
      if (codeText) codeText.textContent = `Kode masalah: ${payload.code}`;
